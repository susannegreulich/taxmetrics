#!/usr/bin/env python3
"""
Official OECD Label Mappings extracted from structure file.

This module contains official code-to-label mappings extracted from OECD's 
structure XML file for various dimensions used in OECD datasets.
"""

def get_official_oecd_mappings():
    """Get official OECD label mappings extracted from structure file"""
    mappings = {}
    
    # ADJUSTMENT mappings (official OECD)
    adjustment_mappings = {
        'AA': 'Amplitude adjusted',
        'C': 'Trend-cycle, calendar adjusted',
        'H': 'Seasonally adjusted and weather normalised',
        'I': 'Irregular component',
        'K': 'Calendar component',
        'M': 'Seasonal and calendar component',
        'N': 'Neither seasonally adjusted nor calendar adjusted',
        'NOR': 'Normalized',
        'R': 'Trend-cycle, not calendar adjusted',
        'RT': 'Ratio to trend',
        'S': 'Seasonally adjusted, not calendar adjusted',
        'T': 'Trend',
        'TR': 'Trend restored',
        'W': 'Calendar adjusted, not seasonally adjusted',
        'X': 'Seasonal component',
        'Y': 'Calendar and seasonally adjusted',
        '_Z': 'Not applicable',
    }

    # REF_AREA mappings (official OECD)
    ref_area_mappings = {
        'AUS': 'Australia',
        'AUT': 'Austria',
        'BEL': 'Belgium',
        'CAN': 'Canada',
        'CHL': 'Chile',
        'COL': 'Colombia',
        'CRI': 'Costa Rica',
        'CZE': 'Czechia',
        'DNK': 'Denmark',
        'EST': 'Estonia',
        'FIN': 'Finland',
        'FRA': 'France',
        'DEU': 'Germany',
        'GRC': 'Greece',
        'HUN': 'Hungary',
        'ISL': 'Iceland',
        'IRL': 'Ireland',
        'ISR': 'Israel',
        'ITA': 'Italy',
        'JPN': 'Japan',
        'KOR': 'Korea',
        'LVA': 'Latvia',
        'LTU': 'Lithuania',
        'LUX': 'Luxembourg',
        'MEX': 'Mexico',
        'NLD': 'Netherlands',
        'NZL': 'New Zealand',
        'NOR': 'Norway',
        'POL': 'Poland',
        'PRT': 'Portugal',
        'SVK': 'Slovak Republic',
        'SVN': 'Slovenia',
        'ESP': 'Spain',
        'SWE': 'Sweden',
        'CHE': 'Switzerland',
        'TUR': 'Türkiye',
        'GBR': 'United Kingdom',
        'USA': 'United States',
        'G4E': 'Major four European countries',
        'E4M': 'Major four euro area countries',
        'G7': 'G7',
        'G20': 'G20',
        'G20XEU': 'G20 excl. European Union',
        'G20OECD': 'G20 OECD countries',
        'G20XOECD': 'G20 non-OECD economies',
        'G20_A': 'G20 advanced economies',
        'G20_E': 'G20 emerging economies',
        'EA': 'Euro area',
        'EA00': 'Euro area (evolving composition)',
        'EA11': 'Euro area (11 countries)',
        'EA12': 'Euro area (12 countries)',
        'EA13': 'Euro area (13 countries)',
        'EA15': 'Euro area (15 countries)',
        'EA16': 'Euro area (16 countries)',
        'EA17': 'Euro area (17 countries)',
        'EA18': 'Euro area (18 countries)',
        'EA19': 'Euro area (19 countries)',
        'EA20': 'Euro area (20 countries)',
        'EA_X': 'Euro area unspecified',
        'EA00_X': 'Euro area (evolving composition) unspecified',
        'EU': 'European Union',
        'EU00': 'European Union (evolving composition)',
        'EU15': 'European Union (15 countries)',
        'EU25': 'European Union (25 countries)',
        'EU27': 'European Union (27 countries)',
        'EU28': 'European Union (28 countries)',
        'EU27_2020': 'European Union (27 countries from 01/02/2020)',
        'EU28XEU15': 'European Union 28 excluding European Union 15 countries',
        'EU_X': 'European Union unspecified',
        'EU00_X': 'European Union (evolving composition) unspecified',
        'EUOECD': 'European Union countries in OECD',
        'EU13OECD': 'European Union (13 countries) in OECD',
        'EU14OECD': 'European Union (14 countries) in OECD',
        'EU19OECD': 'European Union (19 countries) in OECD',
        'EU20OECD': 'European Union (20 countries) in OECD',
        'EU21OECD': 'European Union (21 countries) in OECD',
        'EU22OECD': 'European Union (22 countries) in OECD',
        'EU22OECD_2020': 'European Union (22 countries) in OECD (from 01/02/2020)',
        'EU23OECD': 'European Union (23 countries) in OECD',
        'OECD': 'OECD',
        'OECD00': 'OECD (evolving composition)',
        'OECD26': 'OECD (26 countries)',
        'OECD30': 'OECD (30 countries)',
        'OECD31': 'OECD (31 countries)',
        'OECD34': 'OECD (34 countries)',
        'OECD35': 'OECD (35 countries)',
        'OECD36': 'OECD (36 countries)',
        'OECD37': 'OECD (37 countries)',
        'OECD38': 'OECD (38 countries)',
        'OECDXEA': 'OECD excluding the euro area',
        'OECDXUSAJPNEA17': 'OECD countries except USA, JPN and euro area',
        'OECDA': 'OECD America',
        'OECDS': 'OECD Asia',
        'OECDSO': 'OECD Asia Oceania',
        'OECDE': 'OECD Europe',
        'OECDXG7': 'OECD smaller countries',
        'OECD_X': 'OECD unspecified',
        'OECD00_X': 'OECD (evolving composition) unspecified',
        'OECD_REP': 'OECD average country',
        'WXOECD': 'Non-OECD economies',
        'WXOECDXCHN': 'Non-OECD economies excluding China',
        'AXOECD': 'Non-OECD America',
        'A2XOECD': 'Non-OECD Northern America',
        'A5XOECD': 'Non-OECD Central America and Caribbean',
        'A7XOECD': 'Non-OECD South America',
        'AXOECD_O': 'Other non-OECD America',
        'SXOECD': 'Non-OECD Asia',
        'S3_OXOECD': 'Non-OECD other Western Asia / Near and Middle East countries',
        'S_OXOECD': 'Non-OECD other Asia',
        'EXOECD': 'Non-OECD Europe',
        'OXOECD': 'Non-OECD Oceania',
        'AFG': 'Afghanistan',
        'ALA': 'Åland',
        'ALB': 'Albania',
        'DZA': 'Algeria',
        'ASM': 'American Samoa',
        'AGO': 'Angola',
        'AND': 'Andorra',
        'ATA': 'Antarctica',
        'AIA': 'Anguilla',
        'ATG': 'Antigua and Barbuda',
        'ARG': 'Argentina',
        'ARM': 'Armenia',
        'ABW': 'Aruba',
        'AZE': 'Azerbaijan',
        'BHS': 'Bahamas',
        'BHR': 'Bahrain',
        'GGY': 'Bailiwick of Guernsey',
        'BGD': 'Bangladesh',
        'BRB': 'Barbados',
        'BLR': 'Belarus',
        'BLZ': 'Belize',
        'BEN': 'Benin',
        'BMU': 'Bermuda',
        'BTN': 'Bhutan',
        'BOL': 'Bolivia',
        'BES': 'Bonaire, Saba, Saint Eustatius',
        'BIH': 'Bosnia and Herzegovina',
        'BWA': 'Botswana',
        'BVT': 'Bouvet Island',
        'BRA': 'Brazil',
        'IOT': 'British Indian Ocean Territory',
        'VGB': 'British Virgin Islands',
        'BRN': 'Brunei Darussalam',
        'BGR': 'Bulgaria',
        'BFA': 'Burkina Faso',
        'BDI': 'Burundi',
        'CPV': 'Cabo Verde',
        'KHM': 'Cambodia',
        'CMR': 'Cameroon',
        'CYM': 'Cayman Islands',
        'CAF': 'Central African Republic',
        'TCD': 'Chad',
        'CHN': 'China (People’s Republic of)',
        'CXR': 'Christmas Island',
        'COM': 'Comoros',
        'CCK': 'Cocos (Keeling) Islands',
        'COG': 'Congo',
        'COK': 'Cook Islands',
        'CIV': 'Côte d’Ivoire',
        'HRV': 'Croatia',
        'CUB': 'Cuba',
        'CUW': 'Curaçao',
        'CYP': 'Cyprus',
        'PRK': 'Democratic People’s Republic of Korea',
        'COD': 'Democratic Republic of the Congo',
        'DJI': 'Djibouti',
        'DMA': 'Dominica',
        'DOM': 'Dominican Republic',
        'ECU': 'Ecuador',
        'EGY': 'Egypt',
        'SLV': 'El Salvador',
        'GNQ': 'Equatorial Guinea',
        'ERI': 'Eritrea',
        'SWZ': 'Eswatini',
        'ETH': 'Ethiopia',
        'FLK': 'Falkland Islands (Malvinas)',
        'FRO': 'Faroe Islands',
        'FJI': 'Fiji',
        'GUF': 'French Guiana',
        'PYF': 'French Polynesia',
        'ATF': 'French Southern and Antarctic Lands',
        'GAB': 'Gabon',
        'GMB': 'Gambia',
        'GEO': 'Georgia',
        'GHA': 'Ghana',
        'GIB': 'Gibraltar',
        'GRL': 'Greenland',
        'GRD': 'Grenada',
        'GLP': 'Guadeloupe',
        'GUM': 'Guam',
        'GTM': 'Guatemala',
        'GIN': 'Guinea',
        'GNB': 'Guinea-Bissau',
        'GUY': 'Guyana',
        'HTI': 'Haiti',
        'HMD': 'Heard Island and McDonald Islands',
        'VAT': 'Holy See',
        'HND': 'Honduras',
        'HKG': 'Hong Kong (China)',
        'IND': 'India',
        'IDN': 'Indonesia',
        'IRN': 'Iran',
        'IRQ': 'Iraq',
        'IMN': 'Isle of Man',
        'JAM': 'Jamaica',
        'JEY': 'Jersey',
        'JOR': 'Jordan',
        'KAZ': 'Kazakhstan',
        'KEN': 'Kenya',
        'KIR': 'Kiribati',
        'XKV': 'Kosovo',
        'KWT': 'Kuwait',
        'KGZ': 'Kyrgyzstan',
        'LAO': 'Lao People’s Democratic Republic',
        'LBN': 'Lebanon',
        'LSO': 'Lesotho',
        'LBR': 'Liberia',
        'LBY': 'Libya',
        'LIE': 'Liechtenstein',
        'MAC': 'Macau (China)',
        'MDG': 'Madagascar',
        'MWI': 'Malawi',
        'MYS': 'Malaysia',
        'MDV': 'Maldives',
        'MLI': 'Mali',
        'MLT': 'Malta',
        'MHL': 'Marshall Islands',
        'MTQ': 'Martinique',
        'MRT': 'Mauritania',
        'MUS': 'Mauritius',
        'MYT': 'Mayotte',
        'FSM': 'Micronesia',
        'MDA': 'Moldova',
        'MCO': 'Monaco',
        'MNG': 'Mongolia',
        'MNE': 'Montenegro',
        'MSR': 'Montserrat',
        'MAR': 'Morocco',
        'MOZ': 'Mozambique',
        'MMR': 'Myanmar',
        'NAM': 'Namibia',
        'NRU': 'Nauru',
        'NPL': 'Nepal',
        'NCL': 'New Caledonia',
        'NIC': 'Nicaragua',
        'NER': 'Niger',
        'NGA': 'Nigeria',
        'NIU': 'Niue',
        'NFK': 'Norfolk Island',
        'MKD': 'North Macedonia',
        'MNP': 'Northern Mariana Islands',
        'OMN': 'Oman',
        'PAK': 'Pakistan',
        'PLW': 'Palau',
        'PSE': 'Palestinian Authority or West Bank and Gaza Strip',
        'PAN': 'Panama',
        'PNG': 'Papua New Guinea',
        'PRY': 'Paraguay',
        'PER': 'Peru',
        'PHL': 'Philippines',
        'PCN': 'Pitcairn',
        'PRI': 'Puerto Rico',
        'QAT': 'Qatar',
        'REU': 'Réunion',
        'ROU': 'Romania',
        'RUS': 'Russia',
        'RWA': 'Rwanda',
        'BLM': 'Saint Barthélemy',
        'SHN': 'Saint Helena',
        'KNA': 'Saint Kitts and Nevis',
        'MAF': 'Saint Martin',
        'LCA': 'Saint Lucia',
        'SPM': 'Saint Pierre and Miquelon',
        'VCT': 'Saint Vincent and the Grenadines',
        'WSM': 'Samoa',
        'SMR': 'San Marino',
        'STP': 'Sao Tome and Principe',
        'SAU': 'Saudi Arabia',
        'SEN': 'Senegal',
        'SRB': 'Serbia',
        'SYC': 'Seychelles',
        'SLE': 'Sierra Leone',
        'SGP': 'Singapore',
        'SXM': 'Sint Maarten',
        'SLB': 'Solomon Islands',
        'SOM': 'Somalia',
        'ZAF': 'South Africa',
        'SGS': 'South Georgia and the South Sandwich Islands',
        'SSD': 'South Sudan',
        'LKA': 'Sri Lanka',
        'SDN': 'Sudan',
        'SUR': 'Suriname',
        'SJM': 'Svalbard and Jan Mayen',
        'SYR': 'Syrian Arab Republic',
        'TWN': 'Chinese Taipei',
        'TJK': 'Tajikistan',
        'TZA': 'Tanzania',
        'THA': 'Thailand',
        'TLS': 'Timor-Leste',
        'TGO': 'Togo',
        'TKL': 'Tokelau',
        'TON': 'Tonga',
        'TTO': 'Trinidad and Tobago',
        'TUN': 'Tunisia',
        'TKM': 'Turkmenistan',
        'TCA': 'Turks and Caicos Islands',
        'TUV': 'Tuvalu',
        'UGA': 'Uganda',
        'UKR': 'Ukraine',
        'ARE': 'United Arab Emirates',
        'UMI': 'United States Minor Outlying Islands',
        'VIR': 'United States Virgin Islands',
        'URY': 'Uruguay',
        'UZB': 'Uzbekistan',
        'VUT': 'Vanuatu',
        'VEN': 'Venezuela',
        'VNM': 'Viet Nam',
        'WLF': 'Wallis and Futuna',
        'ESH': 'Western Sahara',
        'YEM': 'Yemen',
        'ZMB': 'Zambia',
        'ZWE': 'Zimbabwe',
        'WXEU15': 'Extra-European Union (15 countries)',
        'WXEU25': 'Extra-European Union (25 countries)',
        'WXEU27': 'Extra-European Union (27 countries)',
        'WXEU28': 'Extra-European Union (28 countries)',
        'WXEU27_2020': 'Extra-European Union (27 countries from 01/02/2020)',
        'YAR_F': 'Former Arab Republic of Yemen',
        'BEL_LUX': 'Belgo-Luxembourg Economic Union',
        'CSK_F': 'Former Czechoslovakia',
        'DDR_F': 'Former Democratic Republic of Germany',
        'VDR_F': 'Former Democratic Republic of Viet Nam',
        'YMD_F': 'Former Democratic Yemen',
        'ETH_F': 'Former Ethiopia',
        'DEU_F': 'Former Federal Republic of Germany',
        'ANT_F': 'Former Netherlands Antilles',
        'ANT_F_ABW': 'Former Netherlands Antilles and Aruba',
        'PCI_F': 'Former Pacific Islands (Trust Territory)',
        'PCZ_F': 'Former Panama Canal Zone',
        'RNY_F': 'Former Rhodesia Nyas',
        'SCG_F': 'Former Serbia and Montenegro',
        'TZA_F': 'Former Tanganyika',
        'SUN_F': 'Former USSR',
        'YUG_F': 'Former Yugoslavia',
        'ACP': 'ACP',
        'AES': 'Advanced economies',
        'APEC': 'APEC',
        'ASEAN': 'ASEAN',
        'CEEC': 'CEEC',
        'CIS': 'CIS',
        'DAE': 'Dynamic Asian economies',
        'EMES': 'Emerging market economies',
        'EEA': 'EEA',
        'GULF': 'Gulf Arabian countries',
        'DAE_IND_IDN': 'Main industrialised Asia',
        'A5M': 'Major five Asia economies',
        'MERCOSUR': 'MERCOSUR',
        'NAFTA': 'NAFTA',
        'OPEC': 'OPEC',
        'OIL_O': 'Other major oil producers',
        'OIL_SAU_O': 'Other oil producers',
        'SAARC': 'SAARC',
        'SDR': 'SDR',
        'DAC': 'DAC countries',
        'ODA': 'ODA recipient countries',
        'ODAA': 'ODA recipient countries - America',
        'ODAF': 'ODA recipient countries - Africa',
        'ODAE': 'ODA recipient countries - Europe',
        'ODAO': 'ODA recipient countries - Oceania',
        'ODAS': 'ODA recipients countries - Asia',
        'LDC': 'Least developed countries',
        'LLDC': 'Land locked developing countries',
        'SIDS': 'Small island developing states',
        'DPGC': 'Developing countries',
        'DPDC': 'Developed countries',
        'MADCT': 'More advanced developed countries and territories',
        'HICS': 'High income countries',
        'UMIC': 'Upper-middle income countries',
        'LMIC': 'Lower-middle income countries',
        'OLIC': 'Other low income countries',
        'INC_X': 'Countries unallocated by income',
        'HICSWB': 'High income countries (World Bank)',
        'UMICWB': 'Upper-middle income countries (World Bank)',
        'LMICWB': 'Lower-middle income countries (World Bank)',
        'OLICWB': 'Low income countries (World Bank)',
        'INCWB_X': 'Countries not classified by the World Bank',
        'HIPC': 'Heavily-indebted poor countries',
        'FSCAC': 'Fragile states and Conflict-affected countries',
        'WBA': 'World aviation bunkers',
        'WBM': 'World maritime bunkers',
        'WB': 'World maritime and aviation bunkers',
        'FZ': 'Free zones',
        'FZ_D': 'Divided zones',
        'HIGH_SEAS': 'High seas',
        'IEA': 'IEA Total',
        'IEAA': 'IEA Americas',
        'IEAOS': 'IEA Asia Oceania',
        'IEAXMEX': 'IEA Total (excl. Mexico)',
        'IEAE': 'IEA Europe',
        'IEA_ACC': 'IEA and Accession/Association countries',
        'ANNEX1': 'Annex I of the United Nations Framework Convention on Climate Change Parties',
        'WXANNEX1': 'Non-Annex I of the United Nations Framework Convention on Climate 
Change Parties',
        'ANNEX2': 'Annex II of the United Nations Framework Convention on Climate Change Parties',
        'ANNEX2A2': 'Annex II of the United Nations Framework Convention on Climate Change North America Parties',
        'ANNEX2E': 'Annex II of the United Nations Framework Convention on Climate Change Europe Parties',
        'ANNEX2OS': 'Annex II of the United Nations Framework Convention on Climate Change Asia Oceania Parties',
        'IPAC': 'International Programme for Action on Climate Parties',
        'A': 'America',
        'A2': 'Northern America',
        'A3': 'Central America',
        'A2_A3_X': 'Northern and Central America unspecified',
        'A4': 'Caribbean',
        'A5': 'Central America and the Caribbean',
        'A5_A7': 'South and Central America',
        'A7': 'South America',
        'A7_X': 'South America unspecified',
        'A8': 'Latin America',
        'A8_M': 'Main Latin American countries',
        'A9': 'Latin America and the Caribbean',
        'A_X': 'America unspecified',
        'A_O': 'Other America',
        'E': 'Europe',
        'E2': 'Eastern Europe',
        'E3': 'Northern Europe',
        'E4': 'Southern Europe',
        'E5': 'Western Europe',
        'E_X': 'Europe unspecified',
        'E_O': 'Other Europe',
        'F': 'Africa',
        'F3': 'Eastern Africa',
        'F4': 'Northern Africa',
        'F5': 'Middle Africa',
        'F6': 'Sub-Saharan Africa',
        'F7': 'Southern Africa',
        'F8': 'Western Africa',
        'F97': 'Middle East',
        'F98': 'Middle East and North Africa',
        'F_X': 'Africa unspecified',
        'F_O': 'Other Africa',
        'O': 'Oceania',
        'O6': 'Australia and New Zealand',
        'O7': 'Melanesia',
        'O8': 'Micronesia',
        'O9': 'Polynesia',
        'O_X': 'Oceania unspecified',
        'O_O': 'Other Oceania',
        'S': 'Asia',
        'S2': 'Eastern Asia',
        'S2_S8': 'Eastern and South-eastern Asia',
        'S3': 'Western Asia / Near and Middle East countries',
        'S3_O': 'Other Western Asia / Near and Middle East countries',
        'S3_X': 'Western Asia unspecified',
        'S4': 'Central Asia',
        'S7': 'Southern Asia',
        'S8': 'South-eastern Asia',
        'S_X': 'Asia unspecified',
        'S_O': 'Other Asia',
        'W_O': 'Other countries',
        'W_O_X': 'Other countries unspecified',
        'WXTIVA': 'Non-target TiVA countries',
        'W': 'World',
        'W_CF': 'World confidential',
        'W_IO': 'World and international organisations',
        'W_X': 'World unspecified',
        'IO': 'International organisations',
        'D': 'Domestic (home or reference area)',
        'WXD': 'Rest of the world',
        '_Z': 'Not applicable',
    }

    # SECTOR mappings (official OECD)
    sector_mappings = {
        '_Z': 'Not applicable',
        'S1': 'Total economy',
        'S11': 'Non-financial corporations',
        'S11A': 'Post office giro',
        'S11003': 'Foreign controlled non-financial corporations',
        'S1100P': 'Private non-financial corporations',
        'S11B': 'Head offices of non-financial corporations',
        'S11DO': 'Domestically controlled non-financial corporations',
        'S11001': 'Public non-financial corporations',
        'S110011': 'Public non-financial corporations, which are part of domestic multinationals',
        'S11001C': 'Public non-financial corporations controlled by central government',
        'S11002': 'National private non-financial corporations',
        'S110021': 'National private non-financial corporations, which are part of domestic multinationals',
        'S12': 'Financial corporations',
        'S12003': 'Foreign controlled financial corporations',
        'S1200P': 'Private financial corporations',
        'S121': 'Central bank',
        'S122': 'Deposit taking corporations, except the Central Bank',
        'S122C': 'Credit institutions',
        'S12201': 'Public deposit taking corporations, except the Central Bank',
        'S12202': 'National private deposit taking corporations, except the Central Bank',
        'S12203': 'Foreign controlled deposit taking corporations, except the Central Bank',
        'S1220P': 'Private deposit-taking corporations',
        'S122A': 'Banks headquartered in the reporting country or currency area',
        'S122B': 'Banks headquartered outside the reporting country or currency area',
        'S122Z': 'Deposit-taking corporations except the central bank and excluding electronic money institutions principally engaged in financial intermediation',
        'S123': 'Money market funds',
        'S12301': 'Public money market funds',
        'S12302': 'National private money market funds',
        'S12303': 'Foreign controlled money market funds',
        'S123A': 'Stable Net Asset Value (NAV) MMFs',
        'S123B': 'Floating NAV MMFs',
        'S124': 'Non-MMF investment funds',
        'S12401': 'Public non-MMF investment funds',
        'S12402': 'National private non-MMF investment funds',
        'S12403': 'Foreign controlled non-MMF investment funds',
        'S124A': 'Open-end non-MMFs',
        'S124A1': 'Non-MMFs Open end funds - Real estate funds',
        'S124A2': 'Non-MMFs Open end funds - Equity funds',
        'S124A3': 'Non-MMFs Open end funds - Bond funds',
        'S124A4': 'Non-MMFs Open end funds - Mixed or balanced funds',
        'S124A5': 'Non-MMFs Open end funds - Hedge funds',
        'S124A9': 'Other non-MMFs Open end funds',
        'S124B': 'Closed-end non-MMFs',
        'S124B1': 'Closed-end Real Estate Funds',
        'S124B2': 'Closed-end Equity funds',
        'S124B3': 'Closed-end Bond funds',
        'S124B4': 'Closed-end Mixed funds',
        'S124B5': 'Closed-end Hedge funds',
        'S124B9': 'Other closed-end non-MMFs',
        'S125': 'Other financial intermediaries, except insurance corporations and pension funds',
        'S12501': 'Public other financial intermediaries, except insurance corporations and pension funds',
        'S12502': 'National private other financial intermediaries, except insurance corporations and pension funds',
        'S12503': 'Foreign controlled other financial intermediaries, except insurance corporations and pension funds',
        'S125A': 'Financial vehicle corporations engaged in securitisation',
        'S125B': 'Financial corporations engaged in lending',
        'S125C': 'Security and derivative dealers',
        'S125D': 'Specialised financial corporations',
        'S125E': 'Other OFIs',
        'S125E1': 'Central clearing counterparties, which are part of other OFIs',
        'S125W': 'Other financial corporations excluding financial vehicle corporations',
        'S126': 'Financial auxiliaries',
        'S12601': 'Public financial auxiliaries',
        'S12602': 'National private financial auxiliaries',
        'S12603': 'Foreign controlled financial auxiliaries',
        'S126B': 'Head offices of financial corporations',
        'S126C': 'Payment institutions',
        'S127': 'Captive financial institutions and money lenders',
        'S12701': 'Public captive financial institutions and money lenders',
        'S12702': 'National private captive financial institutions and money lenders',
        'S12703': 'Foreign controlled captive financial institutions and money lenders',
        'S1271': 'Trusts, estate and agency accounts',
        'S1272': 'Corporate groups’ captive financial entities',
        'S1272A': 'Of which: Foreign owned SPE-type captives',
        'S1273': 'Other captive finance companies and money lenders',
        'S127A': 'Holding companies',
        'S128': 'Insurance corporations',
        'S12801': 'Public insurance corporations',
        'S12802': 'National private insurance corporations',
        'S12803': 'Foreign controlled insurance corporations',
        'S1281': 'Life Insurance Corporations',
        'S1282': 'Non-life Insurance Corporations',
        'S128A': 'Reinsurance corporations',
        'S128B': 'Life insurance corporations excluding reinsurance corporations',
        'S128C': 'Non-life insurance corporations excluding reinsurance corporations',
        'S128D': 'Composite life and non-life insurance corporations excluding reinsurance corporations',
        'S129': 'Pension funds',
        'S12901': 'Public pension funds',
        'S12902': 'National private pension funds',
        'S12903': 'Foreign controlled pension funds',
        'S129A': 'Defined benefit funds',
        'S129B': 'Defined contribution funds',
        'S12A': 'Banks and other financial institutions headquartered in the reporting country or currency area',
        'S12B': 'Banks and other financial institutions headquartered outside the reporting country or currency area',
        'S12C': 'Deposit taking corporations',
        'S12D': 'Financial corporations other than central banks',
        'S12DO': 'Domestically controlled financial corporations',
        'S12001': 'Public financial corporations',
        'S120011': 'Public financial corporations, which are part of domestic multinationals',
        'S12001C': 'Public financial corporations controlled by central government',
        'S12002': 'National private financial corporations',
        'S120021': 'National private financial corporations which are part of domestic multinationals',
        'S12E': 'Electronic money institutions',
        'S12K': 'Monetary financial institutions (MFI)',
        'S12L': 'Investment Funds (MMFs and non-MMFs)',
        'S12L1': 'Real estate funds',
        'S12L2': 'Equity funds',
        'S12L3': 'Bond funds',
        'S12L4': 'Mixed funds',
        'S12L5': 'Hedge funds',
        'S12L9': 'Other funds (other than equity, bond, mixed, real estate and hedge funds)',
        'S12LO': 'Open-end Investment Funds',
        'S12LO1': 'Open-end Real Estate Funds',
        'S12LO2': 'Open-end Equity funds',
        'S12LO3': 'Open-end Bond funds',
        'S12LO4': 'Open-end Mixed funds',
        'S12LO5': 'Open-end Hedge funds',
        'S12LO9': 'Other open-end funds',
        'S12M': 'Financial corporations other than MFIs',
        'S12N': 'Other financial institutions (Financial corporations other than MFIs, insurance corporations, pension funds and financial auxiliaries)',
        'S12O': 'Other financial institutions (Financial corporations other than MFIs, insurance corporations, pension funds and non-MMFs Investment Funds)',
        'S12P': 'Other financial institutions (Financial corporations other than MFIs, insurance corporations and pension funds)',
        'S12Q': 'Insurance corporations and Pension Funds',
        'S12R': 'Other financial corporations',
        'S12R01': 'Other public financial corporations',
        'S12R0P': 'Other private financial corporations',
        'S12S': 'Other Forms of Institutional Savings',
        'S12SP': 'Financial corporations, SPEs',
        'S12T': 'Monetary financial institutions other than central bank',
        'S12V': 'Financial corporations other than central banks and credit institutions',
        'S13': 'General government',
        'S1311': 'Central government',
        'S13111': 'Central government, budgetary units',
        'S13112': 'Central government, extra-budgetary units',
        'S1311B': 'Budgetary Central Government',
        'S1312': 'State government',
        'S13121': 'State government, budgetary units',
        'S13122': 'State government, extra-budgetary units',
        'S1313': 'Local government',
        'S13131': 'Local government, budgetary units',
        'S13132': 'Local government, extra-budgetary units',
        'S1314': 'Social security funds',
        'S13141': 'Social security funds, budgetary units',
        'S13142': 'Social security funds, extra-budgetary units',
        'S1315': 'European institutions and bodies (EU accounts)',
        'S1321': 'Central government including social security',
        'S1322': 'State government including social security',
        'S1323': 'Local government including social security',
        'S133': 'General government non-profit institutions',
        'S1331': 'Central government non-profit institutions',
        'S1332': 'State government non-profit institutions',
        'S1333': 'Local government non-profit institutions',
        'S134': 'General government involved in monetary policy operations',
        'S13L': 'Central and state government',
        'S13M': 'State and local government',
        'S13O': 'General government, except central government',
        'S13P': 'General government, except state government',
        'S13R': 'General government, except local government',
        'S13T': 'General government, except social security funds',
        'S1A': 'Affiliates',
        'S1B': 'Not affiliates',
        'S1F': 'Financial intermediaries (CDIS)',
        'S1G': 'All resident sectors excluding financial intermediaries (CDIS)',
        'S1H': 'Issuer headquartered in the reporting country or currency area',
        'S1K': 'Corporations',
        'S1K1': 'Public corporations',
        'S1KK': 'Central banks and general government',
        'S1KP': 'Private corporations',
        'S1L': 'General government, households and non-profit institutions serving households',
        'S1M': 'Households and non-profit institutions serving households (NPISH)',
        'S14': 'Households',
        'S141': 'Employers',
        'S142': 'Own account workers',
        'S143': 'Employees',
        'S144': 'Recipients of property and transfer income',
        'S1441': 'Recipients of property income',
        'S1442': 'Recipients of pensions',
        'S1443': 'Recipients of other transfers',
        'S15': 'Non-profit institutions serving households',
        'S15002': 'National private',
        'S15003': 'Foreign controlled',
        'S1MU': 'Other households and non-profit institutions serving households (sub-sector not identified)',
        'S1N': 'Not sectorised',
        'S1O': 'Other sectors than MFIs and central government',
        'S1P': 'Other sectors than MFIs and general government',
        'S1Q': 'Other sectors than MFIs',
        'S1R': 'Non-financial sectors, except households',
        'S1S': 'Non-financial corporations, general government and households and NPISH',
        'S1SP': 'Total economy, SPEs',
        'S1SSP': 'Non-financial corporations, SPEs',
        'S1T': 'Non-financial corporations and general government',
        'S1U': 'Non-financial sectors, except central government',
        'S1V': 'Non-financial corporations, households and NPISH',
        'S1W': 'Other sectors than general government',
        'S1X': 'Monetary authorities',
        'S1Y': 'Other sectors than monetary authorities',
        'S1Z': 'Sectors other than deposit-taking corporations and general government (Other Sectors - BPM6)',
        'S1ZK': 'Other sectors than central bank',
        'S1ZL': 'Other sectors than central bank and general government',
        'S1ZM': 'Other sectors than central government',
        'S1ZN': 'Other sectors than state government',
        'S1ZO': 'Other sectors including non-financial corporations, money market funds, non-MMF investment funds, OFIs, insurance corporations, pension funds, general government or a central bank',
        'S1ZP': 'Other sectors than local government',
        'S1ZQ': 'Other sectors than social security funds',
        'S1ZR': 'Other sectors than financial corporations and general government',
        'S1ZS': 'Public sector',
        'S1ZT': 'National private financial and non-financial corporations',
        'S1ZU': 'Sectors other than central bank, credit institutions and general government',
        'S1ZV': 'All sectors other than households and NPISH',
        'SZP': 'Other than general government, clearing and settlement organisations and other financial institutions',
        'SZU': 'Institutions of the European Union (ESA GFS concept)',
        'SZV': 'General government and institutions of the European Union (ESA GFS concept)',
        'S2': 'Rest of the world',
    }

    # UNIT_MEASURE mappings (official OECD)
    unit_measure_mappings = {
        'BBL_US': 'Barrels',
        'C': 'Degrees celsius',
        'CAL': 'Calories',
        'CT': 'Carats',
        'CM': 'Centimetres',
        'D': 'Days',
        'DD': 'Degree days',
        'G': 'Grammes',
        'GT': 'Gross tonnage',
        'GW': 'Gigawatts',
        'GWH': 'Gigawatt hours',
        'H': 'Hours',
        'HA': 'Hectares',
        'HL': 'Hectolitres',
        'KCAL': 'Kilocalories',
        'KG': 'Kilogramme',
        'KJ': 'Kilojoules',
        'KL': 'Kilolitres',
        'KM': 'Kilometres',
        'KM2': 'Square kilometres',
        'KW': 'Kilowatts',
        'KWH': 'Kilowatt hours',
        'L': 'Litres',
        'LB': 'Pounds',
        'M': 'Metres',
        'M2': 'Square metres',
        'M3': 'Cubic metres',
        'MCG': 'Microgrammes',
        'MG': 'Milligrammes',
        'MH': 'Months',
        'MM': 'Millimetres',
        'MN': 'Minutes',
        'MW': 'Megawatts',
        'OZ': 'Ounces',
        'S': 'Seconds',
        'T': 'Tonnes',
        'TJ': 'Terajoules',
        'TWH': 'Terawatt hours',
        'WK': 'Weeks',
        'XGO': '(Gold fine) Troy ounces',
        'Y': 'Years',
        'ANM': 'Animals',
        'BR': 'Births',
        'CH': 'Children',
        'CL': 'Classes',
        'CR': 'Courts',
        'CS': 'Cases',
        'DT': 'Deaths',
        'DSC': 'Discharges',
        'DW': 'Dwellings',
        'ENT': 'Enterprises',
        'ENT_EM': 'Employer enterprises',
        'EST': 'Establishments',
        'EXM': 'Exams',
        'PRC': 'Procedures',
        'FM': 'Families',
        'FTE': 'Full time equivalent unit',
        'HB': 'Inhabitants',
        'HH': 'Households',
        'HH_EQ': 'Equivalised households',
        'IC': 'Inpatient cases',
        'IJ': 'Injuries',
        'INSP': 'Inspectors',
        'INV': 'Inventions',
        'INVS': 'Investors',
        'IT': 'Items',
        'JB': 'Jobs',
        'PAC': 'Packages',
        'PAR': 'Pairs',
        'DOZ': 'Dozens',
        'PAS': 'Passengers',
        'PATN': 'Patents',
        'PATN_FM': 'Patent families',
        'PAT': 'Patients',
        'PR': 'Permits',
        'PS': 'Persons',
        'RCP': 'Recipients',
        'RG': 'Registrations',
        'RO': 'Rooms',
        'RW': 'Registered workplaces',
        'SB': 'Subscriptions',
        'SC': 'Schools',
        'ST': 'Students',
        'UT': 'Units',
        'VC': 'Job vacancies',
        'VIS': 'Visits',
        'VS': 'Vessels',
        'AED': 'United Arab Emirates dirham',
        'AFN': 'Afghani',
        'ALL': 'Lek',
        'AMD': 'Armenian dram',
        'ANG': 'Netherlands Antillian guilder',
        'AOA': 'Kwanza',
        'ARS': 'Argentine peso',
        'AUD': 'Australian dollar',
        'AWG': 'Aruban guilder',
        'AZN': 'Azerbaijanian manat',
        'BAM': 'Bosnia and Herzegovina convertible mark',
        'BBD': 'Barbados dollar',
        'BDT': 'Taka',
        'BGN': 'Bulgarian lev',
        'BHD': 'Bahraini dinar',
        'BIF': 'Burundi franc',
        'BMD': 'Bermudian dollar',
        'BND': 'Brunei dollar',
        'BOB': 'Boliviano',
        'BOV': 'Mvdol',
        'BRL': 'Brazilian real',
        'BSD': 'Bahamian dollar',
        'BTN': 'Ngultrum',
        'BWP': 'Pula',
        'BYN': 'Belarussian ruble',
        'BZD': 'Belize dollar',
        'CAD': 'Canadian dollar',
        'CDF': 'Congolese franc',
        'CENT': 'Euro cent',
        'CFA': 'CFA franc',
        'CHE': 'WIR euro',
        'CHF': 'Swiss franc',
        'CHW': 'WIR franc',
        'CLF': 'Unidad de formento (UF)',
        'CLP': 'Chilean peso',
        'CNY': 'Chinese yuan renminbi',
        'COP': 'Colombian peso',
        'CRC': 'Costa Rican colon',
        'CUP': 'Cuban peso',
        'CVE': 'Cape Verde escudo',
        'CZK': 'Czech koruna',
        'DJF': 'Djibouti franc',
        'DKK': 'Danish krone',
        'DOP': 'Dominican peso',
        'DZD': 'Algerian dinar',
        'EGP': 'Egyptian pound',
        'ERN': 'Nakfa',
        'ETB': 'Ethiopian birr',
        'EUR': 'Euro',
        'FJD': 'Fiji dollar',
        'FKP': 'Falkland Islands pound',
        'GBP': 'Pound sterling',
        'GEL': 'Lari',
        'GHS': 'Ghana cedi',
        'GIP': 'Gibraltar pound',
        'GMD': 'Dalasi',
        'GNF': 'Guinea franc',
        'GTQ': 'Quetzal',
        'GYD': 'Guyana dollar',
        'HKD': 'Hong Kong dollar',
        'HNL': 'Lempira',
        'HRK': 'Croatian kuna',
        'HTG': 'Gourde',
        'HUF': 'Forint',
        'IDR': 'Rupiah',
        'ILS': 'New Israeli sheqel',
        'IMP': 'Isle of Man pounds',
        'INR': 'Indian rupee',
        'IQD': 'Iraqi dinar',
        'IRR': 'Iranian rial',
        'ISK': 'Iceland krona',
        'JMD': 'Jamaican dollar',
        'JOD': 'Jordanian dinar',
        'JPY': 'Yen',
        'KES': 'Kenyan shilling',
        'KGS': 'Som',
        'KHR': 'Riel',
        'KMF': 'Comoro franc',
        'KPW': 'North Korean won',
        'KRW': 'Won',
        'KWD': 'Kuwaiti dinar',
        'KYD': 'Cayman Islands dollar',
        'KZT': 'Tenge',
        'LAK': 'Kip',
        'LBP': 'Lebanese pound',
        'LKR': 'Sri Lanka rupee',
        'LRD': 'Liberian dollar',
        'LSL': 'Loti',
        'LYD': 'Libyan dinar',
        'MAD': 'Moroccan dirham',
        'MDL': 'Moldovan leu',
        'MGA': 'Malagasy ariary',
        'MKD': 'Denar',
        'MMK': 'Kyat',
        'MNT': 'Tugrik',
        'MOP': 'Pataca',
        'MRO': 'Ouguiya',
        'MUR': 'Mauritius rupee',
        'MVR': 'Rufiyaa',
        'MWK': 'Malawian kwacha',
        'MXN': 'Mexican peso',
        'MXV': 'Mexican unidad de inversion (UDI)',
        'MYR': 'Malaysian ringgit',
        'MZN': 'Metical',
        'NAD': 'Namibia dollar',
        'NGN': 'Naira',
        'NIO': 'Cordoba oro',
        'NOK': 'Norwegian krone',
        'NPR': 'Nepalese rupee',
        'NZD': 'New Zealand dollar',
        'OMR': 'Rial Omani',
        'PAB': 'Balboa',
        'PEN': 'Peruvian sol',
        'PGK': 'Kina',
        'PHP': 'Philippine peso',
        'PKR': 'Pakistan rupee',
        'PLN': 'Zloty',
        'PYG': 'Guarani',
        'QAR': 'Qatari rial',
        'RON': 'Romanian leu',
        'RSD': 'Serbian dinar',
        'RUB': 'Russian ruble',
        'RWF': 'Rwanda franc',
        'SAR': 'Saudi riyal',
        'SBD': 'Solomon Islands dollar',
        'SCR': 'Seychelles rupee',
        'SDG': 'Sudanese pound',
        'SEK': 'Swedish krona',
        'SGD': 'Singapore dollar',
        'SHP': 'Saint Helena pound',
        'SLL': 'Leone',
        'SOS': 'Somali shilling',
        'SPL': 'Seborga luigini',
        'SRD': 'Surinam dollar',
        'SSP': 'South-Sudanese pound',
        'STD': 'Dobra',
        'SVC': 'El Salvador colon',
        'SYP': 'Syrian pound',
        'SZL': 'Lilangeni',
        'THB': 'Thai bhat',
        'TJS': 'Somoni',
        'TMT': 'Turkmen manat',
        'TND': 'Tunisian dinar',
        'TOP': 'Tongan paanga',
        'TRY': 'Turkish lira',
        'TTD': 'Trinidad and Tobago dollar',
        'TWD': 'New Taiwan dollar',
        'TZS': 'Tanzanian shilling',
        'UAH': 'Hryvnia',
        'UGX': 'Uganda shilling',
        'USD': 'US dollar',
        'USN': 'US dollar (next day)',
        'USS': 'US dollar (same day)',
        'UYI': 'Uruguay peso en unidades indexadas',
        'UYU': 'Uruguayan peso',
        'UZS': 'Uzbekistan sum',
        'VES': 'Bolivar soberano',
        'VND': 'Dong',
        'VUV': 'Vatu',
        'WST': 'Tala',
        'XAF': 'CFA franc - BEAC',
        'XAG': 'Silver',
        'XAU': 'Gold',
        'XCD': 'East Caribbean dollar',
        'XDC': 'National currency',
        'XDR': 'Special Drawing Rights',
        'XEU': 'European Currency Unit (ECU)',
        'XOF': 'CFA Franc - BCEAO',
        'XPF': 'CFP franc',
        'YER': 'Yemeni rial',
        'ZAR': 'Rand',
        'ZMW': 'Kwacha',
        'RC': 'Reported currency',
        'USD_BBL_US': 'US dollars per barrel',
        'USD_CO2': 'US dollars per unit of CO2',
        'USD_CONT': 'US dollars per container',
        'USD_H': 'US dollars per hour',
        'USD_HH': 'US dollars per household',
        'USD_HHE': 'US dollars per equivalised household',
        'USD_HL': 'US dollars per hectolitre',
        'USD_KG': 'US dollars per kilogramme',
        'USD_L': 'US dollars per litre',
        'USD_PS': 'US dollars per person',
        'USD_ST': 'US dollars per student',
        'USD_T': 'US dollars per tonne',
        'USD_TCO2': 'US dollars per tonne of CO2',
        'USD_TCO2E': 'US dollars per tonne of CO2-equivalent',
        'USD_UT': 'US dollars per unit',
        'USD_PPP': 'US dollars, PPP converted',
        'USD_PPP_H': 'US dollars per hour, PPP converted',
        'USD_PPP_PS': 'US dollars per person, PPP converted',
        'USD_PPP_WR': 'US dollars per worker, PPP converted',
        'USD_EXC': 'US dollars, exchange rate converted',
        'USD_EXC_H': 'US dollars per hour, exchange rate converted',
        'USD_EXC_PS': 'US dollars per person, exchange rate converted',
        'USD_XDC': 'US dollars per national currency',
        'EUR_PPP': 'Euros, PPP converted',
        'EUR_PPP_H': 'Euros, PPP converted, per hour',
        'EUR_PPP_PS': 'Euros, PPP converted, per person',
        'EUR_EXC': 'Euros, exchange rate converted',
        'EUR_EXC_PS': 'Euros, exchange rate converted, per person',
        'EUR_BBL_US': 'Euros per barrel',
        'EUR_CO2': 'Euros per unit of CO2',
        'EUR_H': 'Euros per hour',
        'EUR_HH': 'Euros per household',
        'EUR_HHE': 'Euros per equivalised household',
        'EUR_HL': 'Euros per hectolitre',
        'EUR_KG': 'Euros per kilogramme',
        'EUR_L': 'Euros per litre',
        'EUR_PS': 'Euros per person',
        'EUR_T': 'Euros per tonne',
        'EUR_TCO2': 'Euros per tonne of CO2',
        'EUR_TCO2E': 'Euros per tonne of CO2-equivalent',
        'XDC_BBL_US': 'National currency per barrel',
        'XDC_CAP': 'National currency per unit of capital',
        'XDC_CO2': 'National currency per unit of CO2',
        'XDC_INP': 'National currency per unit of total inputs',
        'XDC_EUR': 'National currency per euro',
        'XDC_H': 'National currency per hour',
        'XDC_HH': 'National currency per household',
        'XDC_HH_EQ': 'National currency per equivalised household',
        'XDC_HL': 'National currency per hectolitre',
        'XDC_KG': 'National currency per kilogramme',
        'XDC_L': 'National currency per litre',
        'XDC_PS': 'National currency per person',
        'XDC_ST': 'National currency per student',
        'XDC_T': 'National currency per tonne',
        'XDC_USD': 'National currency per US dollar',
        'XDC_WR': 'National currency per worker',
        'ADF': 'Andorran franc (1-1 peg to the French franc)',
        'ADP': 'Andorran peseta (1-1 peg to the Spanish peseta)',
        'AFA': 'Afghanistan afghani (old)',
        'AON': 'Angolan kwanza (old)',
        'AOR': 'Angolan kwanza readjustado (old)',
        'ATS': 'Austrian schilling',
        'AZM': 'Azerbaijanian manat (old)',
        'BEF': 'Belgian franc',
        'BEL': 'Belgian franc (financial)',
        'BGL': 'Bulgarian lev (old)',
        'BYB': 'Belarussian rouble (old)',
        'BYR': 'Belarus ruble',
        'COU': 'Unidad de Valor real',
        'CSD': 'Serbian dinar (old)',
        'CYP': 'Cypriot pound',
        'DEM': 'German mark',
        'ECS': 'Ecuador sucre (old)',
        'ECV': 'Unidad de valor constante',
        'EEK': 'Estonian kroon',
        'ESP': 'Spanish peseta',
        'FIM': 'Finnish markka',
        'FRF': 'French franc',
        'GGP': 'Guernsey pounds',
        'GHC': 'Ghanaian cedi (old)',
        'GRD': 'Greek drachma',
        'GWP': 'Guinea-Bissau peso',
        'HKQ': 'Hong Kong dollar (old)',
        'IEP': 'Irish pound',
        'ITL': 'Italian lira',
        'JEP': 'Jersey pounds',
        'LTL': 'Lithuanian litas',
        'LUF': 'Luxembourg franc',
        'LVL': 'Latvian lats',
        'MGF': 'Malagasy franc',
        'MTL': 'Maltese lira',
        'MXP': 'Mexican peso (old)',
        'MZM': 'Mozambique metical (old)',
        'NLG': 'Netherlands guilder',
        'PLZ': 'Polish zloty (old)',
        'PTE': 'Portugese escudo (old)',
        'ROL': 'Romanian leu (old)',
        'RUR': 'Russian ruble (old)',
        'SDD': 'Sudanese dinar (old)',
        'SDP': 'First sudanese pound',
        'SIT': 'Slovenian tolar',
        'SKK': 'Slovak koruna',
        'SRG': 'Suriname guilder (old)',
        'TJR': 'Tajikistan rouble (old)',
        'TMM': 'Turkmenistan manat (old)',
        'TPE': 'East Timor escudo (old)',
        'TRL': 'Turkish lira (old)',
        'TVD': 'Tuvalu dollars',
        'VEB': 'Venezuela bolivar (old)',
        'VEF': 'Bolivar fuerte',
        'XFO': 'Gold-franc',
        'XFU': 'UIC-franc',
        'YUM': 'Yugoslav dinar (old)',
        'ZMK': 'Zambian kwacha',
        'ZWD': 'Zimbabwe dollar',
        'ZWL': 'Fourth zimbabwe dollar',
        'ZWN': 'Zimbabwe dollars (old)',
        'ZWR': 'Third zimbabwe dollar',
        'BBL_US_D': 'Barrels per day',
        'BBL_US_T': 'Barrels per tonne',
        'CAL_PS_D': 'Calories per person per day',
        'CS_10P5PS': 'Cases per 100 000 persons',
        'CS_BD': 'Cases per available bed',
        'CS_PS': 'Cases per person',
        'CS_WR': 'Cases per worker',
        'CN_PS': 'Consultations per person',
        'D_IJ': 'Days per injury',
        'D_PS': 'Days per person',
        'D_WK': 'Days per week',
        'D_WR': 'Days per worker',
        'D_Y': 'Days per year',
        'D_Y_PS': 'Days per year per person',
        'DSC_10P5PS': 'Discharges per 100 000 persons',
        'DT_10P3BR': 'Deaths per 1 000 births',
        'EXM_10P3PS': 'Exams per 1 000 persons',
        'EXM_SCN': 'Exams per scanner',
        'G_PS': 'Grammes per person',
        'G_PS_D': 'Grammes per person per day',
        'H_D': 'Hours per day',
        'H_PS': 'Hours per person',
        'H_WK': 'Hours per week',
        'H_WR': 'Hours per worker',
        'H_Y': 'Hours per year',
        'H_Y_PS': 'Hours per year per person',
        'HB_KM2': 'Inhabitants per square kilometre',
        'IJ_10P6HB': 'Injured per 1 000 000 inhabitants',
        'INSP_EMP': 'Inspectors per employed person',
        'KBBL_US_D': 'Kilobarrels per day',
        'KCAL_PS': 'Kilocalories per person',
        'KCAL_PS_D': 'Kilocalories per person per day',
        'KG_10P3USD': 'Kilogrammes per 1 000 US dollars',
        'KG_10P6USD': 'Kilogrammes per 1 000 000 US dollars',
        'KG_CO2E_KG': 'Kilogrammes of CO2-equivalent per kilogramme',
        'KG_CO2_USD': 'Kilogrammes of CO2 per US dollar',
        'KG_D': 'Kilogrammes per day',
        'KG_EUR': 'Kilogrammes per euro',
        'KG_HA': 'Kilogrammes per hectare',
        'KG_PS': 'Kilogrammes per person',
        'KG_PS_D': 'Kilogrammes per person per day',
        'KG_PS_WK': 'Kilogrammes per person per week',
        'KG_PS_Y': 'Kilogrammes per person per year',
        'KG_HH': 'Kilogrammes per household',
        'KG_HH_WK': 'Kilogrammes per household per week',
        'KG_USD': 'Kilogrammes per US dollar',
        'KG_WK': 'Kilogrammes per week',
        'KGOE': 'Kilogrammes of oil equivalent',
        'KGOE_PS': 'Kilogrammes of oil equivalent per person',
        'KJ_KG': 'Kilojoules per kilogramme',
        'KJ_M3': 'Kilojoules per cubic metre',
        'KM_100KM2': 'Kilometres per 100 square kilometres',
        'L_PS': 'Litres per person',
        'L_HH': 'Litres per household',
        'M2_PS': 'Square metres per person',
        'M3_PS': 'Cubic metres per person',
        'MCG_L': 'Microgrammes per litre',
        'MCG_M3': 'Microgrammes per cubic metre',
        'MG_L': 'Milligrammes per litre',
        'MM_H': 'Millimetres per hour',
        'MM_Y': 'Millimetres per year',
        'MN_D': 'Minutes per day',
        'PASKM': 'Passenger-kilometres',
        'PRC_10P5PS': 'Procedures per 100 000 persons',
        'PRC_10P5M': 'Procedures per 100 000 men',
        'PRC_10P5W': 'Procedures per 100 000 women',
        'PRC_10P3POP_Y_GE65': 'Procedures per 1 000 population aged 65 years old and over',
        'PS_FTE': 'Persons (full-time equivalent)',
        'PS_H': 'Persons per hour',
        'PS_KM2': 'Persons per square kilometer',
        'RO_PS': 'Rooms per person',
        'ST_CL': 'Students per class',
        'T_10P3USD': 'Tonnes per 1 000 US dollars',
        'T_10P6USD': 'Tonnes per 1 000 000 US dollars',
        'T_ANM': 'Tonnes per animal',
        'KT_CO2': 'Kilotonnes of CO2',
        'T_CO2': 'Tonnes of CO2',
        'KT_CO2E': 'Kilotonnes of CO2-equivalent',
        'T_CO2E': 'Tonnes of CO2-equivalent',
        'T_CO2E_PS': 'Tonnes of CO2-equivalent per person',
        'T_CO2E_USD': 'Tonnes of CO2-equivalent per US dollar',
        'T_HA': 'Tonnes per hectare',
        'T_PS': 'Tonnes per person',
        'T_NMVOCE': 'Tonnes of NMVOC-equivalent',
        'T_NMVOCE_PS': 'Tonnes of NMVOC-equivalent per person',
        'T_NMVOCE_USD': 'Tonnes of NMVOC equivalent per US dollar',
        'T_NO2E': 'Tonnes of NO2-equivalent',
        'T_NO2E_PS': 'Tonnes of NO2-equivalent per person',
        'T_NO2E_USD': 'Tonnes of NO2-equivalent per US dollar',
        'T_RME': 'Tonnes in raw material equivalents',
        'T_SO2E': 'Tonnes of SO2-equivalent',
        'T_SO2E_PS': 'Tonnes of SO2-equivalent per person',
        'T_SO2E_USD': 'Tonnes of SO2-equivalent per US dollar',
        'T_USD': 'Tonnes per US dollar',
        'TKM': 'Tonnes-kilometres',
        'TOE': 'Tonnes of oil equivalent',
        'TOE_10P3USD': 'Tonnes of oil equivalent per 1 000 US dollars',
        'TOE_PS': 'Tonnes of oil equivalent per person',
        'VIS_INSP': 'Visits per inspector',
        'WK_Y': 'Weeks per year',
        'Y_10P5PS': 'Years per 100 000 persons',
        'MD': 'Man-days',
        'MY': 'Man-years',
        '100HB': 'Per 100 inhabitants',
        '10P3HB': 'Per 1 000 inhabitants',
        '10P3HB_AGE': 'Per 1 000 inhabitants in the same age group',
        '10P5HB': 'Per 100 000 inhabitants',
        '10P6HB': 'Per 1 000 000 inhabitants',
        '10P3EMP': 'Per 1 000 employment',
        '10P3LF': 'Per 1 000 of labour force',
        'PT_POP': 'Percentage of population',
        'PT_POP_AGE': 'Percentage of population in the same age',
        'PT_POP_SEX_AGE': 'Percentage of population in the same sex and age',
        'PT_POP_SUB': 'Percentage of population in the same subgroup',
        'PT_POP_Y_LT1': 'Percentage of children aged less than 1 years',
        'PT_POP_Y_GE15': 'Percentage of population aged 15 years or over',
        'PT_POP_Y15T74': 'Percentage of population aged 15-74 years',
        'PT_POP_Y20T64': 'Percentage of population aged 20-64 years',
        'PT_POP_Y_GE65': 'Percentage of population aged 65 years or over',
        'PT_WAP': 'Percentage of working age population',
        'PT_WAP_SUB': 'Percentage of working age population in the same subgroup',
        'PT_CH': 'Percentage of children',
        'PT_SC': 'Percentage of schools',
        'PT_ST': 'Percentage of students',
        'PT_ST_SUB': 'Percentage of students in the same subgroup',
        'PT_LF': 'Percentage of labour force',
        'PT_LF_SUB': 'Percentage of labour force in the same subgroup',
        'PT_EMP': 'Percentage of employment',
        'PT_EMP_SUB': 'Percentage of employment in the same subgroup',
        'PT_SAL': 'Percentage of employees',
        'PT_SAL_SUB': 'Percentage of employees in the same subgroup',
        'PT_SELF': 'Percentage of self-employed',
        'PT_SELF_SUB': 'Percentage of self-employed in the same subgroup',
        'PT_UNEMPT': 'Percentage of unemployment',
        'PT_UNEMPT_SUB': 'Percentage of unemployment in the same subgroup',
        'PT_UNE': 'Percentage of unemployed',
        'PT_ENT': 'Percentage of enterprises',
        'PT_ENT_EM': 'Percentage of employer enterprises',
        'PT_SME': 'Percentage of small and medium-sized enterprises',
        'PT_EST': 'Percentage of establishments',
        'PT_JB': 'Percentage of jobs',
        'PT_EMP_FT': 'Percentage of full-time employment',
        'PT_WG_SAL_PA': 'Percentage of prime-age employees wage',
        'PT_WG_A_SAL': 'Percentage of average wage',
        'PT_WG_A_SAL_FT': 'Percentage of average wage of full-time employees',
        'PT_WG_SAL_M_D': 'Percentage of wages of men in the same decile',
        'PT_EARN': 'Percentage of earnings',
        'PT_RSH': 'Percentage of researchers',
        'PT_AR': 'Percentage of area',
        'PT_BAR': 'Percentage of built-up area',
        'PT_FAR': 'Percentage of forested area',
        'PT_LAR': 'Percentage of land area',
        'PT_ALAR': 'Percentage of agricultural land area',
        'PT_ARL': 'Percentage of arable land',
        'PT_EEZ': 'Percentage of exclusive economic zone',
        'PT_HH': 'Percentage of households',
        'PT_AS_S14': 'Percentage of household assets',
        'PT_AS_S1M': 'Percentage of household and NPISH assets',
        'PT_FAS_S14': 'Percentage of household financial assets',
        'PT_FAS_S1M': 'Percentage of household and NPISH financial assets',
        'PT_B1GQ': 'Percentage of GDP',
        'PT_B1GQ_GR': 'Percentage of GDP growth',
        'PT_PB1GQ': 'Percentage of potential GDP',
        'PT_B1G': 'Percentage of gross value added',
        'PT_B1N': 'Percentage of net value added',
        'PT_B2A3G': 'Percentage of gross operating surplus and mixed income',
        'PT_B2G': 'Percentage of gross operating surplus',
        'PT_B2N': 'Percentage of net operating surplus',
        'PT_B90_S14': 'Percentage of household wealth',
        'PT_BF90_S14': 'Percentage of household financial wealth',
        'PT_B90_S1M': 'Percentage of household and NPISH wealth',
        'PT_BF90_S1M': 'Percentage of household and NPISH financial wealth',
        'PT_B5G': 'Percentage of gross national income',
        'PT_B5G_S14': 'Percentage of balance of household gross primary income',
        'PT_B5N_S14': 'Percentage of balance of household net primary incomes',
        'PT_B6G_S14': 'Percentage of household gross disposable income',
        'PT_B6G_S1M': 'Percentage of household and NPISH gross disposable income',
        'PT_B6GA_S1M': 'Percentage of household and NPISH gross disposable income adjusted for the net change in pension entitlements',
        'PT_B6N_S14': 'Percentage of household net disposable income',
        'PT_B6N_S1M': 'Percentage of household and NPISH net disposable income',
        'PT_B6NA_S1M': 'Percentage of household and NPISH net disposable income adjusted for the net change in pension entitlements',
        'PT_B7G_S14': 'Percentage of household gross adjusted disposable income',
        'PT_B7N_S14': 'Percentage of household net adjusted disposable income',
        'PT_FD4_S13': 'Percentage of general government debt',
        'PT_ODA_S13': '[Deprecated] Percentage of tax revenue / total tax receipt',
        'PT_ODA': 'Percentage of tax receipts',
        'PT_EXP': 'Percentage of total expenditure',
        'PT_BERD': 'Percentage of business enterprise expenditure on R&D',
        'PT_GBARD': 'Percentage of government allocations for R&D',
        'PT_GBARD_C': 'Percentage of civil government allocations for R&D',
        'PT_GERD': 'Percentage of gross domestic expenditure on R&D',
        'PT_GOVERD': 'Percentage of government intramural expenditure on R&D',
        'PT_HERD': 'Percentage of higher education expenditure on R&D',
        'PT_OTE_S13': 'Percentage of general government expenditure',
        'PT_OTR_S13': 'Percentage of general government revenues',
        'PT_P1': 'Percentage of gross output',
        'PT_P2': 'Percentage of intermediate consumption',
        'PT_P2_ACT': 'Percentage of intermediate consumption in the same activity',
        'PT_P2_PROD': 'Percentage of intermediate consumption in the same product',
        'PT_P2_ACT_PROD': 'Percentage of intermediate consumption in the same activity and product',
        'PT_P3': 'Percentage of final consumption expenditure',
        'PT_P3_S14': 'Percentage of final household consumption expenditure',
        'PT_P3_S14_O_PROD': 'Percentage of household final consumption expenditure at purchasers\' prices in the same product',
        'PT_P3_S14_PROD': 'Percentage of household final consumption expenditure in the same product',
        'PT_P3T6': 'Percentage of final demand',
        'PT_P51G': 'Percentage of gross fixed capital formation',
        'PT_P51G_PROD': 'Percentage of gross fixed capital formation in the same product',
        'PT_P6': 'Percentage of exports',
        'PT_P6_PROD': 'Percentage of exports in the same product',
        'PT_P6_W': 'Percentage of world exports',
        'PT_P7': 'Percentage of imports',
        'PT_P7_W': 'Percentage of world imports',
        'PT_TU': 'Percentage of total demand / use',
        'PT_TS': 'Percentage of total supply',
        'PT_TS_O_PROD': 'Percentage of total supply at purchasers\' prices in the same product',
        'PT_TRD_W': 'Percentage of world trade',
        'PT_TRD_D': 'Percentage of domestic trade',
        'PT_COS': 'Percentage of costs',
        'PT_PCOS': 'Percentage of production costs',
        'PT_FDI': 'Percentage of foreign direct investment',
        'PT_APD': 'Percentage of official development assistance',
        'PT_CA': 'Percentage of current account',
        'PT_GS': 'Percentage of goods and services',
        'PT_ADM': 'Percentage of admissions',
        'PT_PAT': 'Percentage of patients',
        'PA': 'Percent per annum',
        'PB': 'Percentage balance',
        'PC': 'Percentage change',
        'PD': 'Percentage points',
        'PD_EMP_PA': 'Percentage point difference from prime-age workers',
        '0_TO_1': '0-1 scale',
        'PO': 'Points',
        'FCTR_D1': 'Factor of decile 1',
        'FCTR_D5': 'Factor of decile 5',
        'IX': 'Index',
        'GR': 'Growth rate',
        '_X': 'Unspecified',
    }

    # TRANSFORMATION mappings (official OECD)
    transformation_mappings = {
        'A1': 'Average, period on period',
        'A12': '12-period moving average',
        'A3': '3-period moving average',
        'A4': '4-period moving average',
        'A6': '6-period moving average',
        'ACR': 'Annual rate of change corrected for the effect of methodological changes where applicable',
        'AS1999': 'Average since 1999',
        'AW12': 'Weighted average, over 12 periods',
        'C12': '12-period cumulated sum',
        'C16': '16 period cumulated sum',
        'C3': '3-period cumulated sum',
        'C4': '4-period cumulated sum',
        'C4G': 'Growth rate, period on period, over 4-period cumulated sum',
        'C6': '6-period cumulated sum',
        'CY': 'Cumulated sum, over 1 year',
        'D1': 'Differences, period on period, first order',
        'D1D1': 'Differences, period on period, second order',
        'D4': 'Difference period on 4 periods, first order',
        'DY': 'Difference, over 1 year',
        'ECX': 'Monthly index, backdated, ECU (to 1989) and fixed euro conversion rate (from 1990) used for weights',
        'EPX': 'Monthly index, backdated, PPP exchange rates used for weights',
        'ERX': 'Monthly index, backdated, fixed euro conversion rate used for weights',
        'F1': 'Growth rate, flow over stock, period on period',
        'F12': 'Growth rate, flow over stock, over 12 periods',
        'F3': 'Growth rate, flow over stock, over 3 periods',
        'F4': 'Growth rate, flow over stock, over 4 periods',
        'F6': 'Growth rate, flow over stock, over 6 periods',
        'FO1': 'Contribution to growth rate, flow over stock, period on period',
        'FO12': 'Contribution to growth rate, flow over stock, over 12  periods',
        'FO16': 'Contribution to growth rate, flow over stock, over 16 periods',
        'FO3': 'Contribution to growth rate, flow over stock, over 3 periods',
        'FO4': 'Contribution to growth rate, flow over stock, over 4 periods',
        'FO6': 'Contribution to growth rate, flow over stock, over 6 periods',
        'FY': 'Growth rate, flow over stock, over 1 year',
        'G1': 'Growth rate, period on period',
        'G10': 'Growth rate, over 10 periods',
        'G3': 'Growth rate, over 3 periods',
        'G3Y': 'Growth rate, over 3 years',
        'G4Y': 'Growth rate, over 4 years',
        'G6': 'Growth rate, over 6 periods',
        'GC10Y': 'Compound growth rate, over 10 years',
        'GC5Y': 'Compound growth rate, over 5 years',
        'GCM': 'Cumulative growth rate since base period',
        'GO1': 'Contribution to growth rate, period on period',
        'GO3': 'Contribution to growth rate, over 3 periods',
        'GO6': 'Contribution to growth rate, over 6 periods',
        'GOY': 'Contribution to growth rate, over 1 year',
        'GR': 'Growth rate, over reference year',
        'GY': 'Growth rate, over 1 year',
        'LA': 'Annual levels',
        'N': 'Non transformed data',
        'R1': 'Revisions, difference to last transmission period',
        'GYA3': 'Growth rate, over 1 year, based on 3-period average',
        'GYA12': 'Growth rate, over 1 year, based on 12-period average',
        'IX': 'Index',
        'MIX': 'Multilateral index',
        'MIX_1': 'Multilateral index rebased to 1',
        'MIX_100': 'Multilateral index rebased to 100',
        '_Z': 'Not applicable',
    }

    # FREQ mappings (official OECD)
    freq_mappings = {
        'A': 'Annual',
        'A2': 'Biennial',
        'A3': 'Triennial',
        'A4': 'Quadrennial',
        'A5': 'Quinquennial',
        'A10': 'Decennial',
        'A20': 'Bidecennial',
        'A30': 'Tridecennial',
        'A_3': 'Three times a year',
        'S': 'Half-yearly, semester',
        'Q': 'Quarterly',
        'M': 'Monthly',
        'M2': 'Bimonthly',
        'M_2': 'Semimonthly',
        'M_3': 'Three times a month',
        'W': 'Weekly',
        'W2': 'Biweekly',
        'W3': 'Triweekly',
        'W4': 'Four-weekly',
        'W_2': 'Semiweekly',
        'W_3': 'Three times a week',
        'D': 'Daily',
        'D_2': 'Twice a day',
        'H': 'Hourly',
        'H2': 'Bihourly',
        'H3': 'Trihourly',
        'B': 'Daily – business week',
        'N': 'Minutely',
        'I': 'Irregular',
        'OA': 'Occasional annual',
        'OM': 'Occasional monthly',
        '_O': 'Other',
        '_U': 'Unspecified',
        '_Z': 'Not applicable',
    }

    # INSTR_ASSET mappings (official OECD)
    instr_asset_mappings = {
        '_Z': 'Not applicable',
        'B1GC': 'GDP rolling sum',
        'B90': 'Net worth',
        'BF90': 'Net financial worth',
        'BF90A': 'Net financial worth adjusted for pension liabilities',
        'BF90_POP': 'Net financial worth, per capita',
        'B9F': 'Net financial transactions',
        'F': 'Total financial instruments',
        'F_POP': 'Total financial instruments, per capita',
        'F_NG': 'Total financial instruments and gross non-financial assets',
        'F1': 'Monetary gold and Special Drawing Rights (SDRs)',
        'F11': 'Monetary gold',
        'F12': 'Special Drawing Rights (SDRs)',
        'F2': 'Currency and deposits',
        'F21': 'Currency',
        'F22': 'Transferable deposits',
        'F221': 'Inter-bank positions',
        'F229': 'Other transferable deposits',
        'F29': 'Other deposits',
        'F2M': 'Deposits',
        'F2T4': 'Currency and deposits, debt securities and loans',
        'F2T4S': 'Short-term liabilities',
        'F3': 'Debt securities',
        'F3T4': 'Debt securities and loans',
        'F3RES': 'Debt securities issued by residents',
        'F3NONRES': 'Debt securities issued by non-residents',
        'F4': 'Loans',
        'F41': 'Credit lines',
        'F4A': 'Consumer credit',
        'F4A1': 'Revolving credit',
        'F4A2': 'Non-revolving credit',
        'F4B': 'Loans for house purchasing',
        'F4C': 'Loans for other purpose',
        'F4D': 'Repurchase agreements, securities lending and margin lending insofar as recorded as loans',
        'F4M2': 'Non-performing loans',
        'F4RES': 'Loans granted to residents',
        'F4NONRES': 'Loans granted to non-residents',
        'F5': 'Equity and investment fund shares/units',
        'F51': 'Equity',
        'F511': 'Listed shares',
        'F512': 'Unlisted shares',
        'F519': 'Other equity',
        'F51M': 'Unlisted shares and other equity',
        'F52': 'Investment fund shares/units',
        'F521': 'Money market fund shares/units',
        'F522': 'Non-money market funds investment fund shares/units',
        'F5221': 'Real estate fund shares',
        'F5222': 'Bond fund shares',
        'F5223': 'Mixed fund shares',
        'F5224': 'Equity fund shares',
        'F5229': 'Other fund shares',
        'F5RES': 'Equity and investment fund shares issued by residents',
        'F5NONRES': 'Equity and investment fund shares issued by non-residents',
        'F6': 'Insurance, pension and standardized guarantee schemes',
        'F61': 'Non-life insurance technical reserves',
        'F62': 'Life insurance and annuity entitlements',
        'F62A': 'Life insurance and annuity entitlements, of which unit linked',
        'F62B': 'Life insurance and annuity entitlements, of which non-unit-linked',
        'F63': 'Pension entitlements',
        'F63A': 'Pension entitlements, of which defined contribution',
        'F63A1': 'Pension entitlements, defined contribution, managed by autonomous pension funds',
        'F63A2': 'Pension entitlements, defined contribution, managed by non-autonomous pension funds',
        'F63A3': 'Pension entitlements, defined contribution, managed by insurers',
        'F63B': 'Pension entitlements, of which defined benefit',
        'F63B1': 'Pension entitlements, defined benefit, managed by autonomous pension funds',
        'F63B2': 'Pension entitlements, defined benefit, managed by non-autonomous pension funds',
        'F63B3': 'Pension entitlements, defined benefit, managed by insurers',
        'F63C': 'Pension entitlements, of which hybrid schemes',
        'F63C1': 'Pension entitlements, hybrid schemes, managed by autonomous pension funds',
        'F63C2': 'Pension entitlements, hybrid schemes, managed by non-autonomous pension funds',
        'F63C3': 'Pension entitlements, hybrid schemes, managed by insurers',
        'F63F': 'Pension entitlements, managed by autonomous pension funds',
        'F63H': 'Pension entitlements, managed by non-autonomous pension funds',
        'F63I': 'Pension entitlements, managed by insurers',
        'F63O': 'Other pension plans, including unfunded pension plans',
        'F64': 'Claims of pension funds on pension managers',
        'F65': 'Entitlements to non-pension benefits',
        'F66': 'Provisions for calls under standardized guarantees',
        'F6M': 'Pension entitlements, claims of pension funds on pension managers and entitlements to non-pension benefits',
        'F6O': 'Non-life insurance technical provisions and provisions for calls under standardized guarantees',
        'F7': 'Financial derivatives and employee stock options',
        'F71': 'Financial derivatives',
        'F711': 'Options type',
        'F712': 'Forward type',
        'F72': 'Employee stock options',
        'F8': 'Other accounts receivable/payable',
        'F81': 'Trade credits and advances',
        'F89': 'Other accounts receivable/payable, excluding trade credits and advances',
        'FD2': 'SDRs, currency and deposits, debt securities and loans',
        'FD3': 'SDRs, currency and deposits, debt securities, loans and other accounts payable',
        'FD4': 'Total gross debt',
        'FLD': 'Total gross debt held by domestic creditors',
        'FLE': 'Total gross debt held by external creditors',
        'FO2': 'Investment fund shares/units, life insurance, and pension',
        'FR3': 'Securities, currency and deposits',
        'FR3A': 'Securities, except investment fund shares, currency and deposits',
        'GD': 'Maastricht debt',
        'N11G': 'Fixed assets, gross',
        'N11N': 'Fixed assets, net',
        'N11KG': 'Total construction (Buildings and structures), gross',
        'N11KN': 'Total construction (Buildings and structures), net',
        'N111G': 'Dwellings, gross',
        'N111N': 'Dwellings, net',
        'N112G': 'Other buildings and structures, gross',
        'N112N': 'Other buildings and structures, net',
        'N1121G': 'Buildings other than dwellings, gross',
        'N1121N': 'Buildings other than dwellings, net',
        'N1122G': 'Other structures, gross',
        'N1122N': 'Other structures, net',
        'N1123N': 'Land improvements, net',
        'N11MG': 'Machinery and equipment and weapons systems, gross',
        'N11MN': 'Machinery and equipment and weapons systems, net',
        'N1131G': 'Transport equipment, gross',
        'N1131N': 'Transport equipment, net',
        'N1132G': 'ICT equipment, gross',
        'N1132N': 'ICT equipment, net',
        'N1132G_N11OG': 'ICT equipment and other machinery and equipment and weapons systems, gross',
        'N11321G': 'Computer hardware, gross',
        'N11321N': 'Computer hardware, net',
        'N11322G': 'Telecommunications equipment, gross',
        'N11322N': 'Telecommunications equipment, net',
        'N11OG': 'Other machinery and equipment and weapons systems, gross',
        'N11ON': 'Other machinery and equipment and weapons systems, net',
        'N1139G': 'Other machinery and equipment, gross',
        'N1139N': 'Other machinery and equipment, net',
        'N115G': 'Cultivated biological resources, gross',
        'N115N': 'Cultivated biological resources, net',
        'N117G': 'Intellectual property products, gross',
        'N117N': 'Intellectual property products, net',
        'N1171G': 'Research and development, gross',
        'N1171N': 'Research and development, net',
        'N1172G': 'Mineral exploration and evaluation, gross',
        'N1172N': 'Mineral exploration and evaluation, net',
        'N1173G': 'Computer software and databases, gross',
        'N1173N': 'Computer software and databases, net',
        'N11731G': 'Computer software, gross',
        'N11731N': 'Computer software, net',
        'N11732G': 'Databases, gross',
        'N11732N': 'Databases, net',
        'N1174G': 'Entertainment, literary or artistic originals, gross',
        'N1174N': 'Entertainment, literary or artistic originals, net',
        'N1179G': 'Other intellectual property products, gross',
        'N1179N': 'Other intellectual property products, net',
        'N12G': 'Inventories, gross',
        'N12N': 'Inventories, net',
        'N13G': 'Valuables, gross',
        'N13N': 'Valuables, net',
        'N1G': 'Produced non-financial assets, gross',
        'N1MG': 'Inventories and valuables, gross',
        'N1N': 'Produced non-financial assets, net',
        'N1OG': 'Fixed assets and inventories, gross',
        'N1ON': 'Fixed assets and inventories, net',
        'N2111G': 'Land underlying buildings and structures, gross',
        'N2111N': 'Land underlying buildings and structures, net',
        'N211G': 'Land, gross',
        'N211N': 'Land, net',
        'N212G': 'Mineral and energy reserves, gross',
        'N212N': 'Mineral and energy reserves, net',
        'N215G': 'Other natural resources, gross',
        'N215N': 'Other natural resources, net',
        'N21G': 'Natural resources, gross',
        'N21N': 'Natural resources, net',
        'N21OG': 'Non-cultivated biological resources and water resources, gross',
        'N21ON': 'Non-cultivated biological resources and water resources, net',
        'N22G': 'Contracts, leases and licences, gross',
        'N22N': 'Contracts, leases and licences, net',
        'N23G': 'Purchases less sales of goodwill and marketing assets, gross',
        'N23N': 'Purchases less sales of goodwill and marketing assets, net',
        'N2G': 'Non-produced non-financial assets, gross',
        'N2KG': 'Intangible non-produced assets, gross',
        'N2KN': 'Intangible non-produced assets, net',
        'N2N': 'Non-produced non-financial assets, net',
        'NG': 'Total non-financial assets, gross',
        'NN': 'Total non-financial assets, net',
        'N11TAG': 'Tangible fixed assets, gross',
        'N11TAN': 'Tangible fixed assets, net',
        'NN_POP': 'Total non-financial assets, per capita, net',
        'N111N_POP': 'Dwellings, per capita, net',
        'N211N_POP': 'Land, per capita, net',
        'NKN_POP': 'Non-financial assets, other than dwellings and lands, per capita, net',
        'N211121N': 'Land underlying buildings other than dwellings, net',
        'N211121G': 'Land underlying buildings other than dwellings, gross',
        'N21111N': 'Land underlying dwellings, net',
        'N21111G': 'Land underlying dwellings, gross',
    }

    # TABLE_IDENTIFIER mappings (official OECD)
    table_identifier_mappings = {
        'PSD': 'Table PSD - Public sector debt',
        'R15_FIN_SF': 'G-20 recommendation 15 framework - financial stocks and flows',
        'R15_NONFIN_S': 'G-20 recommendation 15 framework - stocks of non-financial assets',
        'R15_NONFIN_T': 'G-20 recommendation 15 framework - non-financial transactions',
        'T01GDP': 'Table 01GDP - GDP, components and breakdowns',
        'T01EMP': 'Table 01EMP - Employment and breakdown by industry and population',
        'T0101': 'Table 0101 - Gross value added at basic prices and gross domestic product at market prices',
        'T0101T0103': 'Tables 0101, 0102 and 0103',
        'T0102': 'Table 0102 - GDP identity from the expenditure side',
        'T0103': 'Table 0103 - GDP identity from the income side',
        'T0107': 'Table 0107 - Disposable income, saving, net lending / borrowing',
        'T0110': 'Table 0110 - Population and employment',
        'T0111': 'Table 0111 - Employment by industry',
        'T0117': 'Table 0117 - Final consumption expenditure of households by durability',
        'T0117_0501_0502': 'Tables 0117, 0501 and 0502',
        'T0119': 'Table 0119 - Simplified non-financial accounts by institutional sector',
        'T0120': 'Table 0120 - Exports of goods (fob) and services by Member States of the EU / third countries.',
        'T0121': 'Table 0121 - Imports of goods (fob) and services by Member States of the EU/third countries',
        'T0199': 'Table 01xx - Source from various table 01 sub-tables',
        'T0200': 'Table 0200 - Main aggregates of general government',
        'T0301': 'Table 0301- Output and income',
        'T0302': 'Table 0302 - Capital formation',
        'T0302_2200': 'Tables 0302 and 2200',
        'T0303': 'Table 0303 - Employment',
        'T0501': 'Table 0501 - Final consumption expenditure of households by purpose',
        'T0502': 'Table 0502 - Final consumption expenditure of households',
        'T0610': 'Table 0610 - Financial accounts by sector (transactions), consolidated',
        'T0611': 'Table 0611 - Other change in volume accounts, consolidated',
        'T0612': 'Table 0612 - Revaluation of financial instruments accounts, consolidated',
        'T0620': 'Table 0620 - Financial accounts by sector (transactions), non-consolidated',
        'T0621': 'Table 0621 - Other change in volume accounts, non-consolidated',
        'T0622': 'Table 0622 - Revaluation of financial instruments accounts, non-consolidated',
        'T0625': 'Table 0625 - Financial accounts by sector (transactions), counterpart information, non-consolidated',
        'T0710': 'Table 0710 - Balance sheets for financial assets and liabilities (stocks), consolidated',
        'T0720': 'Table 0720 - Balance sheets for financial assets and liabilities (stocks), non-consolidated',
        'T0725': 'Table 0725 - Balance sheets for financial assets and liabilities (stocks), counterpart information, non-consolidated',
        'T0800': 'Table 0800 - Non-financial accounts by sector - annual',
        'T0801': 'Table 0801 - Non-financial accounts by sector - quarterly',
        'T0801SA': 'Table 0801SA - QSA Quarterly sector accounts: seasonally adjusted and volume data',
        'T0900': 'Table 0900 - Detailed tax and social contribution receipts by type of tax or social contribution and receiving sub-sector including the list of taxes and social contributions according to national classification',
        'T0999': 'Table 0999 - Questionnaire NTL - Detailed list of taxes and social contributions according to national classification',
        'T1001': 'Table 1001 - Tables by region (NUTS II)',
        'T1002': 'Table 1002 - Tables by industry and by region (NUTS II)',
        'T1100': 'Table 1100 - General Government expenditure by function',
        'T1200': 'Table 1200 - Tables by industry, A6 and by region (NUTS III)',
        'T1300': 'Table 1300 - Households accounts by region (NUTS II)',
        'T1500': 'Table 1500 - Supply table at basic prices, including a transformation into purchasers\' prices',
        'T1600': 'Table 1600 - Use Table at purchasers\' prices',
        'T1601': 'Table 1601 - Use table for domestic input at purchasers\' prices',
        'T1602': 'Table 1602 - Use table for imports at purchasers\' prices',
        'T1610': 'Table 1610 - Use table at basic prices',
        'T1611': 'Table 1611 - Use table for domestic input at basic prices',
        'T1612': 'Table 1612 - Use table for imports at basic prices',
        'T1620': 'Table 1620 - Trade and transport margins',
        'T1630': 'Table 1630 - Taxes less subsidies on products',
        'T1631': 'Table 1631 - Taxes less subsidies on products - excluding VAT',
        'T1632': 'Table 1632 - Value added tax',
        'T1633': 'Table 1633 - Taxes on products',
        'T1634': 'Table 1634 - Subsidies on products',
        'T1700': 'Table 1700 - Symmetric Input-output table (product*product)',
        'T1750': 'Table 1700 - Symmetric Input-output table (industry*industry)',
        'T1800': 'Table 1800 - Symmetric Input-output table for domestic production (product*product)',
        'T1850': 'Table 1850 - Symmetric Input-output table for domestic production (industry*industry)',
        'T1900': 'Table 1900 - Symmetric Input-output table for imports (product*product)',
        'T1950': 'Table 1950 - Symmetric Input-output table for imports (industry*industry)',
        'T2000': 'Table 2000 - Cross-classification of fixed assets by industry and by asset',
        'T2200': 'Table 2200 - Cross-classification of gross fixed capital formation by industry and by asset',
        'T2500': 'Table 2500 - Quarterly non-financial accounts for general government',
        'T2600': 'Table 2600 - Balance sheets for non-financial assets',
        'T2700': 'Table 2700 - Quarterly financial accounts of general government',
        'T2800': 'Table 2800 - Quarterly government debt (Maastricht debt)',
        'T2899': 'Table 2899 - Quarterly intergovernmental lending by counter-party government',
        'T2900': 'Table 2900 - Pension schemes in social insurance: base case',
        'T2901': 'Table 2901 - Pension schemes in social insurance: sensitivity analysis 1',
        'T2902': 'Table 2902 - Pension schemes in social insurance: sensitivity analysis 2',
        'T7HH': 'Table 7HH - Households\' financial assets and liabilities',
        'T7II': 'Table 7II - Institutional investors\' assets and liabilities',
        'TSB720': 'Table SB720 - Non-bank financial intermediation (financial stocks), non-consolidated',
        'T001': 'Table 001 - Exchange rates and PPPs',
        'T750': 'Government debt in nominal values, consolidated',
    }

    # TRANSACTION mappings (official OECD)
    transaction_mappings = {
        '_X': 'Unspecified',
        '_Z': 'Not applicable',
        'B1GQ': 'Gross domestic product',
        'P3T5': 'Domestic demand',
        'P3': 'Final consumption expenditure',
        'P31': 'Individual consumption expenditure',
        'P3S13': 'General government final consumption expenditure',
        'P32': 'Collective consumption expenditure',
        'P41': 'Actual individual consumption',
        'P5': 'Gross capital formation',
        'P51G': 'Gross fixed capital formation',
        'B1G': 'Value added, gross',
        'D1': 'Compensation of employees',
        'D11': 'Wages and salaries',
        'D12': 'Employers\' social contributions',
        'B2A3G': 'Operating surplus and mixed income, gross',
        'D21X31': 'Taxes less subsidies on products',
        'D2X3': 'Taxes on production and imports less subsidies',
        'D2': 'Taxes on production and imports',
        'D2_D5_D91_D611_D613': 'Total tax receipts and actual social contributions',
        'D3': 'Subsidies on production and imports',
        'P5M': 'Changes in inventories and acquisitions less disposals of valuables',
        'P52': 'Changes in inventories',
        'P53': 'Acquisitions less disposals of valuables',
        'D21': 'Taxes on products',
        'D31': 'Subsidies on products',
        'B11': 'External balance of goods and services',
        'P6': 'Exports of goods and services',
        'P61': 'Exports of goods',
        'P62': 'Exports of services',
        'P7': 'Imports of goods and services',
        'P71': 'Imports of goods',
        'P72': 'Imports of services',
        'YA0': 'Statistical discrepancy (expenditure approach)',
        'YA1': 'Statistical discrepancy (production approach)',
        'YA2': 'Statistical discrepancy (income approach)',
        'B101': 'Changes in net worth due to saving and capital transfers',
        'B12': 'Current external balance',
        'B1GXP119': 'Value added, gross, excluding FISIM',
        'B1N': 'Value added, net',
        'B1NQ': 'Net domestic product at market prices',
        'B2A3N': 'Operating surplus and mixed income, net',
        'B2G': 'Operating surplus, gross',
        'B2N': 'Operating surplus, net',
        'B3G': 'Mixed income, gross',
        'B4G': 'Entrepreneurial income, gross',
        'B5G': 'Balance of primary incomes, gross / National income, gross',
        'B5N': 'Balance of primary incomes, net / National income, net',
        'B6G': 'Disposable income, gross',
        'B6N': 'Disposable income, net',
        'B7G': 'Adjusted disposable income, gross',
        'B7N': 'Adjusted disposable income, net',
        'B8G': 'Saving, gross',
        'B8N': 'Saving, net',
        'B9': 'Net lending (+) / net borrowing (-)',
        'B90': 'Net worth',
        'B9F': 'Net financial transactions',
        'B9FX9': 'Discrepancy between the financial and non-financial net lending / borrowing',
        'BF90': 'Net financial worth',
        'BF90A': 'Net financial worth adjusted for pension liabilities',
        'D211': 'Value added type taxes',
        'D212': 'Taxes and duties on imports excluding Value Added Tax',
        'D2121': 'Import duties',
        'D2122': 'Taxes on imports excluding Value Added Tax and duties',
        'D2122A': 'Levies on imported agricultural products',
        'D2122B': 'Monetary compensatory amounts on imports',
        'D2122C': 'Excise duties',
        'D2122D': 'General sales taxes',
        'D2122E': 'Taxes on specific services',
        'D2122F': 'Profits of import monopolies',
        'D214': 'Taxes on products except Value Added Tax, import and export taxes',
        'D214A': 'Excise duties and consumption taxes',
        'D214B': 'Stamp taxes',
        'D214C': 'Taxes on financial and capital transactions',
        'D214D': 'Car registration taxes',
        'D214E': 'Taxes on entertainment',
        'D214F': 'Taxes on lotteries, gambling and betting',
        'D214G': 'Taxes on insurance premiums',
        'D214H': 'Other taxes on specific services',
        'D214I': 'General sales or turnover taxes',
        'D214J': 'Profits of fiscal monopolies',
        'D214K': 'Export duties and monetary compensatory amounts collected on exports',
        'D214L': 'Other taxes on products n.e.c.',
        'D29': 'Other taxes on production',
        'D29A': 'Taxes on land, buildings or other structures',
        'D29B': 'Taxes on the use of fixed assets',
        'D29C': 'Total wage bill and payroll taxes',
        'D29D': 'Taxes on international transactions',
        'D29E': 'Business and professional licences',
        'D29F': 'Taxes on pollution',
        'D29G': 'Under-compensation of Value Added Tax (flat rate system)',
        'D29H': 'Other taxes on production n.e.c.',
        'D29X39': 'Other taxes less other subsidies on production',
        'D39': 'Other subsidies on production',
        'D4': 'Property income',
        'D41': 'Interest',
        'D41G': 'Interest before FISIM allocation',
        'D42': 'Distributed income of corporations',
        'D421': 'Dividends',
        'D422': 'Withdrawals from income of quasi-corporations',
        'D43': 'Reinvested earnings on FDI (excluding IF)',
        'D44': 'Other investment income',
        'D441': 'Investment income attributable to insurance policyholders',
        'D442': 'Investment income payable on pension entitlements',
        'D443': 'Investment income attributable to collective investment fund share holders',
        'D45': 'Rent',
        'D4N': 'Property income other than interest',
        'D5': 'Current taxes on income, wealth, etc.',
        'D51': 'Taxes on income',
        'D51A': 'Taxes on individual or household income excluding holding gains',
        'D51B': 'Taxes on the income or profits of corporations excluding holding gains',
        'D51C': 'Taxes on holding gains',
        'D51C1': 'Taxes on individual or household holding gains',
        'D51C2': 'Taxes on holding gains of corporations',
        'D51C3': 'Other taxes on holding gains n.e.c.',
        'D51D': 'Taxes on winnings from lottery or gambling',
        'D51E': 'Other taxes on income n.e.c.',
        'D51M': 'Taxes on individual or household income including holding gains',
        'D51O': 'Taxes on the income or profits of corporations including holding gains',
        'D59': 'Other current taxes',
        'D59A': 'Current taxes on capital',
        'D59B': 'Poll taxes',
        'D59C': 'Expenditure taxes',
        'D59D': 'Payments by households for licences',
        'D59E': 'Taxes on international transactions',
        'D59F': 'Other current taxes n.e.c.',
        'D6': 'Social contributions and social benefits',
        'D61': 'Net social contributions',
        'D611': 'Employers\' actual social contributions',
        'D6111': 'Employers\' actual pension contributions',
        'D6112': 'Employers\' actual non-pension contributions',
        'D611C': 'Compulsory employers\' actual social contributions',
        'D611V': 'Voluntary employers\' actual social contributions',
        'D612': 'Employers\' imputed social contributions',
        'D6121': 'Employers\' imputed pension contributions',
        'D6122': 'Employers\' imputed non-pension contributions',
        'D613': 'Households\' actual social contributions',
        'D613CE_VE': 'Employees\' social contributions',
        'D613VE': 'Voluntary employees\' social contributions',
        'D613CSN_VSN': 'Social contributions by self- and non-employed persons',
        'D613CSN': 'Compulsory social contributions by self- and non-employed persons',
        'D613VSN': 'Voluntary social contributions by self- and non-employed persons',
        'D6131': 'Households\' actual pension contributions',
        'D6132': 'Households\' actual non-pension contributions',
        'D613C': 'Compulsory households\' actual social contributions',
        'D613CE': 'Compulsory employees\' actual social contributions',
        'D613CN': 'Compulsory actual social contributions by the non-employed',
        'D613CS': 'Compulsory actual social contributions by the self-employed',
        'D613V': 'Voluntary households\' actual social contributions',
        'D614': 'Households\' social contribution supplements',
        'D6141': 'Households\' pension contribution supplements',
        'D6142': 'Households\' non-pension contribution supplements',
        'D619': 'Other (actuarial) change of pension entitlements in social security pension schemes',
        'D61N': 'Actual social contributions from employers and households',
        'D61SC': 'Social insurance scheme service charges (-)',
        'D62': 'Social benefits other than social transfers in kind',
        'D62_63': 'Social benefits and social transfers in kind',
        'D63': 'Social transfers in kind',
        'D631': 'Social transfers in kind - non-market production',
        'D632': 'Social transfers in kind - purchased market production',
        'D6L': 'Social contributions and social benefits, other than social transfer in kind',
        'D6M': 'Social benefits other than social transfers in kind and social transfers in kind, purchased market production',
        'D7': 'Other current transfers',
        'D71': 'Net non-life insurance premiums',
        'D72': 'Non-life insurance claims',
        'D73': 'Current transfers within general government',
        'D74': 'Current international cooperation',
        'D75': 'Miscellaneous current transfers',
        'D751': 'Current transfers to NPISHs',
        'D76': 'Value Added Tax and gross national income - based EU own resources',
        'D7N': 'Current international cooperation, miscellaneous current transfers and Value Added Tax and gross national income - based EU own resources',
        'D8': 'Adjustment for the change in pension entitlements',
        'D81': 'Transfer of pension entitlements between schemes',
        'D82': 'Change in entitlements due to negotiated changes in scheme structure',
        'D9': 'Capital transfers',
        'D91': 'Capital taxes',
        'D91A': 'Taxes on capital transfers',
        'D91B': 'Capital levies',
        'D91C': 'Other capital taxes n.e.c.',
        'D92': 'Investment grants',
        'D99': 'Other capital transfers',
        'D995': 'Capital transfers from general government to relevant sectors representing taxes and social contributions assessed but unlikely to be collected',
        'D995A': 'Taxes on products assessed but unlikely to be collected',
        'D995B': 'Other taxes on production assessed but unlikely to be collected',
        'D995C': 'Taxes on income assessed but unlikely to be collected',
        'D995D': 'Other current taxes assessed but unlikely to be collected',
        'D995E': 'Employers actual social contributions assessed but unlikely to be collected',
        'D995F': 'Households\' actual social contributions assessed but unlikely to be collected',
        'D995FE': 'Employees\' actual social contributions assessed but unlikely to be collected',
        'D995FN_D995FS': 'Social contributions by self- and non-employed persons assessed but unlikely to be collected',
        'D995FN': 'Actual social contributions by non-employed persons assessed but unlikely to be collected',
        'D995FS': 'Actual social contributions by self-employed persons assessed but unlikely to be collected',
        'D995G': 'Capital taxes assessed but unlikely to be collected',
        'D9N': 'Other capital transfers and investment grants',
        'EMP': 'Total employment',
        'F': 'Transactions in financial assets and liabilities',
        'K': 'Changes in positions other than transactions',
        'K5': 'Other changes in volume not elsewhere classified',
        'K7': 'Revaluations / Nominal holding gains and losses',
        'KA': 'Other changes excluding revaluations',
        'LE': 'Closing balance sheet/positions/stocks',
        'LS': 'Opening balance sheet/positions/stocks',
        'NP': 'Acquisitions less disposals of non-financial non-produced assets',
        'ODA': 'Total tax receipts',
        'ODC': 'Total receipts from taxes and net social contributions (including imputed social contributions) after deduction of amounts assessed but unlikely to be collected',
        'ODD': 'Tax burden corresponding to total receipts from taxes and compulsory social contributions after deduction of amounts assessed but unlikely to be collected',
        'OEC': 'Intermediate consumption, Other taxes on production, Current taxes on income, wealth, etc., Adjustment for the change in pension entitlements',
        'OED': 'Other taxes on production, Current taxes on income, wealth, etc., Adjustment for the change in pension entitlements',
        'OTE': 'Total government expenditure',
        'OTEK': 'Total expenditure other than Compensation of employees, Social benefits and social transfers in kind for products supplied to HH via market producers, Intermediate consumption and Gross fixed capital formation',
        'OTGL': 'Trading gain or loss',
        'OTR': 'Total government revenue',
        'OTTM': 'Trade and transport margins',
        'OTTM1': 'Trade margins',
        'OTTM2': 'Transport margins',
        'P1': 'Output',
        'P11': 'Market output',
        'P119': 'Financial Intermediation Services Indirectly Measured (FISIM)',
        'P12': 'Output for own final use',
        'P13': 'Non-market output',
        'P131': 'Payments for other non-market output',
        'P132': 'Other non-market output, n.e.c.',
        'P1M': 'Market output and output for own final use',
        'P1O': 'Market output, output for own final use and payments for other non-market output',
        'P2': 'Intermediate consumption',
        'P311': 'Final consumption expenditure of durable goods',
        'P312': 'Final consumption expenditure of semi-durable goods',
        'P313': 'Final consumption expenditure of non-durable goods',
        'P314': 'Final consumption expenditure of services',
        'P31K': 'Final consumption expenditure of semi-durable goods, non-durable goods, and services',
        'P3T6': 'Total demand',
        'P51C': 'Consumption of fixed capital',
        'P51N': 'Net fixed capital formation',
        'P5L': 'Gross capital formation and acquisitions less disposals of non-produced assets',
        'P5N': 'Net capital formation',
        'P61B': 'Export on second hand goods',
        'P61C': 'Merchanting',
        'P62F': 'Exports of FISIM',
        'P6A': 'Re-exports',
        'P72F': 'Imports of FISIM',
        'POP': 'Total population',
        'PTOT': 'Term of trade',
        'SAL': 'Employees',
        'SELF': 'Self employed',
        'TR1': 'Total resources production account',
        'TR211': 'Total resources generation of income account',
        'TR212': 'Total resources allocation of primary income account',
        'TR22': 'Total resources secondary distribution of income account',
        'TR241': 'Total resources use of disposable income account',
        'TR311': 'Total change in liabilities and net worth, change in net worth due to saving and capital transfers account',
        'TR312': 'Total change in liabilities and net worth, acquisitions of non-financial assets accounts',
        'TS': 'Total supply',
        'TU': 'Total use',
        'TU1': 'Total uses production account',
        'TU211': 'Total uses generation of income account',
        'TU212': 'Total uses allocation of primary income account',
        'TU22': 'Total uses secondary distribution of income account',
        'TU241': 'Total uses use of disposable income account',
        'TU311': 'Total change in assets, change in net worth due to saving and capital transfers account',
        'TU312': 'Total change in assets, acquisitions of non-financial assets accounts',
        'P3_P51G': 'Final domestic demand excluding inventories',
        'P33': 'Direct purchases abroad by residents',
        'P7ADJ': 'CIF/FOB adjustment on imports',
        'P3S15': 'NPISHs final consumption expenditure',
        'D612_D614': 'Imputed social contributions from employers and households',
        'P34': 'Domestic purchases by non-residents',
        'P51CB': 'Consumption of fixed capital, capital account',
        'B1GQXOTGL': 'Gross domestic income',
        'PPP_B1GQ': 'Purchasing Power Parities for GDP',
        'PPP_P31S14': 'Purchasing Power Parities for household final consumption expenditure',
        'PPP_P41': 'Purchasing Power Parities for actual individual consumption',
        'EXC_A': 'Exchange rates, average',
        'B5N_POP': 'Balance of primary incomes, net / National income, net, per capita',
        'EXC_E': 'Exchange rates, end of period',
        'B1GQ_POP': 'Gross domestic product, per capita',
        'P3_POP': 'Final consumption expenditure, per capita',
        'P31_POP': 'Individual consumption expenditure, per capita',
        'P41_POP': 'Actual individual consumption, per capita',
        'P31NC': 'Final consumption expenditure of residents on the territory and abroad',
        'P31DC': 'Final consumption expenditure of residents and non-residents on the territory',
        'P3S14DC': 'Households final consumption expenditure domestic concept',
        'B5N_R': 'Real balance of primary incomes, net / Real national income, net',
        'IN1C': 'Primary income receivable from the rest of the world',
        'B6G_POP': 'Disposable income, gross, per capita',
        'IN1D': 'Primary income payable to the rest of the world',
        'B7G_POP': 'Adjusted disposable income, gross, per capita',
        'IN1B': 'Net primary income from the rest of the world',
        'B6N_R': 'Real household disposable income, net',
        'IN21C': 'Current transfers receivable from the rest of the world',
        'B7N_R': 'Real household adjusted disposable income, net',
        'IN21D': 'Current transfers payable to the rest of the world',
        'D91_OTHR': 'Capital taxes and other revenues',
        'IN21B': 'Net current transfers from the rest of the world',
        'B8G_POP': 'Saving, gross, per capita',
        'D9C': 'Capital transfers receivable from the rest of the world',
        'B5G_POP': 'Balance of primary incomes, gross / National income, gross, per capita',
        'D9D': 'Capital transfers payable to the rest of the world',
        'D61SC_P': 'Pension scheme service charges (-)',
        'D9B': 'Net capital transfers from the rest of the world',
        'SA1': 'Production account',
        'SA2': 'Generation of income account',
        'SA3_SA4': 'Distribution of income accounts',
        'SA3': 'Allocation of primary income account',
        'SA4': 'Secondary distribution of income account',
        'SA5': 'Use of disposable income account',
        'SA6': 'Capital account',
        'SA61': 'Changes in net worth due to saving and capital transfers account',
        'SA62': 'Acquisition of non-financial assets account',
    }

    # Add all mappings to the main dictionary
    mappings["ADJUSTMENT"] = adjustment_mappings

    mappings["REF_AREA"] = ref_area_mappings

    mappings["SECTOR"] = sector_mappings

    mappings["UNIT_MEASURE"] = unit_measure_mappings

    mappings["TRANSFORMATION"] = transformation_mappings

    mappings["FREQ"] = freq_mappings

    mappings["INSTR_ASSET"] = instr_asset_mappings

    mappings["TABLE_IDENTIFIER"] = table_identifier_mappings

    mappings["TRANSACTION"] = transaction_mappings

    return mappings
